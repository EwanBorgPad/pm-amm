...
function SidebarItem({ label, count, active, onClick }: SidebarItemProps) {
  return (
    ...
    >
      <span>{label}</span>
      {count !== undefined && <span className="text-muted text-[11px]">{count}</span>}
    </div>
  );
}
...
export function Sidebar({
  ...
  return (
    <aside
      className={`border-r border-line py-[16px] font-mono text-[12px] hidden lg:block transition-all duration-[200ms] ${
    ...
    >
      ...
      {!collapsed && (
        <>
          ...
          <SidebarItem
            label="My positions"
            count={positionCount || undefined}
            active={filter === "positions"}
