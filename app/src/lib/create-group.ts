...
export async function runCreateGroup({
  ...
  // Random 48-bit IDs to prevent front-running: a sequential scheme like
  // `baseId, baseId+1, …, baseId+N` lets a watcher who sees the first tx
  // predict every leg PDA and race the authority by submitting
  // `initialize_market(id=predicted, authority=attacker)` to grief the
  // attach phase. 48 bits stays under Number.MAX_SAFE_INTEGER (2^53) so we
  // don't need BigInt round-trips, and 2^48 ≈ 2.8e14 collision space is
  // overkill for any realistic deploy cadence.
  ...
  // 2. Each leg: init + deposit + attach bundled in ONE tx. Each marketId is
  //    drawn independently so a front-runner who learns groupId AND one
  //    legMarketId cannot infer the others.
...
/**
 * Generate a cryptographically random 48-bit integer, suitable for use as a
 * Solana account PDA seed. 48 bits because:
 *   - It fits in a JS `number` (< 2^53), so callers can pass it around
 *     without BN ceremony.
 *   - 2^48 ≈ 2.8 × 10^14 — the birthday-paradox collision probability is
 *     ~50% only after ~16M IDs, which dwarfs any realistic creation rate.
 *   - It's unpredictable to a front-runner watching the mempool.
 */
